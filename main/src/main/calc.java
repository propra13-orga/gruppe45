		...
		if (calc.ingame) {
			...
			for (spielfeld.hero hase: board.heros){
		        ...
		        for (spielfeld.killerbunny killerhase: board.killers){
		        	...
		        	if (killerhase.pos_y>Main.FRAMESIZE_Y-board.rand_x-held_hoehe || killerhase.pos_y<board.rand_x) {
						killerhase.move_y = killerhase.move_y*killerhase.geschwindigkeit* (-1);
					}
		        	// killerbunny collision with frame x value
		        	if (killerhase.pos_x>Main.FRAMESIZE_X-board.rand_y-held_breite || killerhase.pos_x<board.rand_y) {
