...
public class Koll {
	...
	public boolean ziel(){
		if ( (Spielfeld.obj_list.get(1).pos_x < ziel.pos_x+ziel.image.getWidth(null)) && (Spielfeld.obj_list.get(1).pos_x+hase.image.getWidth(null) > ziel_pos_x) && (hase.pos_y < ziel.pos_y+ziel.image.getHeight(null)) && (hase.pos_y+hase.image.getHeight(null) > ziel.pos_y) ){
			return false;
		}
		else return true;
	}
	...
	public Boolean koll(Objekt tester){
		for(int i; i < Spielfeld.obj_list.size(); i++){
